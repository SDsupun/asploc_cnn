...
int init_qlenet_uart(INTC *IntcInstPtr)
{
	...
	Status = UartPsIntrQLenet(IntcInstPtr, &UartPs,
				UART_DEVICE_ID, UART_INT_IRQ_ID);
	if (Status != XST_SUCCESS) {
		xil_printf("UART Interrupt initialization Failed\r\n");
		return XST_FAILURE;
	}

	xil_printf("Successfully initialized UART Interrupt\r\n");
	return XST_SUCCESS;
}
...
/**
*
* This function initialize the hardware configs
*
...
int recieve_image(u8 *rxBuff){
	...
	/*
	 * Initialize the receive buffer bytes to a known value for
	 * verify
	 */
	 ...
	 */
	XUartPs_Recv(&UartPs, rxBuff, TEST_BUFFER_SIZE);
